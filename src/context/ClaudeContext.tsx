...
export const ClaudeProvider: React.FC<ClaudeProviderProps> = ({ children }) => {
  ...
  const initialize = useCallback(async () => {
    try {
      ...
      // Get API key from environment variable (VITE_ANTHROPIC_API_KEY) or localStorage
      const apiKey = getClaudeApiKey();

      if (!apiKey) {
        throw new Error('Claude API key not found. Please add your Anthropic API key in .env file (VITE_ANTHROPIC_API_KEY).');
      }
